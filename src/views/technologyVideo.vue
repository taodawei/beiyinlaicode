            ...
            <div class="ctx-right">
              ...
              <div class="video-box">
                <div class="video-inner">
                  <div class="modal-video-jifen" v-if="showVideoJifen">
                    <img
                      src="@img/close.png"
                      alt=""
                      class="img-close"
                      @click="showVideoJifen = false"
                    />
                    <img src="@img/video-done.png" alt="" class="img-done" />
                    <div class="text">
                      恭喜您增加 <b>{{ add_jifen }}</b> 积分
                    </div>
                  </div>
...
export default {
  ...
  data() {
    return {
      ...
      is_zengsong: 0, //是否已经赠送积分
      add_jifen: 0, //当前视频加的积分
    };
  },
  computed: {
    ...mapState([""]),
  ...
  methods: {
    setView() {
      this.$api("brand_studyInfo", {
        ...
        this.is_zengsong = res.data.is_zengsong;
        this.initVideo();
      });
    },
    ...
    queryVideoJifen() {
      ...
      if (!this.is_zengsong) {
        this.$api("brand_addJifen", {
          id: this.$route.query.id,
        }).then((res) => {
          //console.log("增加积分", res);
          if (res.code == 1) {
            this.is_zengsong = 1;
          }
          this.showVideoJifen = true;
          this.add_jifen = res.data.jifen;
        });
      }
    },
...
<style scoped lang="less" src="@/assets/h5css/page/technologyVideo.less"></style>
